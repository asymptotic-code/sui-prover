...
pub struct MemoryUsage {
    ...
    pub direct: SetDomain<QualifiedInstId<DatatypeId>>,
    // The memory transitively used in either the function itself or at least one of its callees.
    pub transitive: SetDomain<QualifiedInstId<DatatypeId>>,
    // The union of the above sets
    pub all: SetDomain<QualifiedInstId<DatatypeId>>,
}
...
impl MemoryUsage {
    ...
    fn add_direct(&mut self, mem: QualifiedInstId<DatatypeId>) {
        self.direct.insert(mem.clone());
        self.all.insert(mem);
    }

    fn add_transitive(&mut self, mem: QualifiedInstId<DatatypeId>) {
    ...
    pub fn get_direct_inst(&self, inst: &[Type]) -> BTreeSet<QualifiedInstId<DatatypeId>> {
        self.direct
            .iter()
    ...
    pub fn get_transitive_inst(&self, inst: &[Type]) -> BTreeSet<QualifiedInstId<DatatypeId>> {
        self.transitive
            .iter()
            .map(|mem| mem.instantiate_ref(inst))
            .collect()
    ...
    pub fn get_all_inst(&self, inst: &[Type]) -> BTreeSet<QualifiedInstId<DatatypeId>> {
        self.all
            .iter()
            .map(|mem| mem.instantiate_ref(inst))
            .collect()
    ...
    pub fn get_direct_uninst(&self) -> BTreeSet<QualifiedId<DatatypeId>> {
        self.direct
            .iter()
    ...
    pub fn get_transitive_uninst(&self) -> BTreeSet<QualifiedId<DatatypeId>> {
        self.transitive
            .iter()
            .map(|mem| mem.module_id.qualified(mem.id))
            .collect()
    ...
    pub fn get_all_uninst(&self) -> BTreeSet<QualifiedId<DatatypeId>> {
        self.all
            .iter()
            .map(|mem| mem.module_id.qualified(mem.id))
            .collect()
...
macro_rules! generate_inserter {
    ($field: ident, $method: ident) => {
        paste! {
            #[allow(dead_code)]
            fn [<$method _ $field>](&mut self, mem: QualifiedInstId<DatatypeId>) {
            ...
            fn [<$method _ $field _iter>](
                &mut self,
                mems: impl Iterator<Item = QualifiedInstId<DatatypeId>>
            ) {
                for mem in mems {
