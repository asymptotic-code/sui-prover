...
impl SpecPurityAnalysis {
            ...
            for func in module.get_functions() {
                ...
                if let Some(info) = purity_info {
                    ...
                    if info.is_pure {
                        ...
                        // let generated_code = func_target.get_bytecode();

                        // writeln!(writer, "  Generated Bytecode:")?;
                        // for (idx, bc) in generated_code.iter().enumerate() {
                        //     writeln!(writer, "    {}: {:?}", idx, bc)?;
                        // }
                ...
                Bytecode::Call(attr, _, operation, _, _) => {
                    ...
                    match operation {
                        Operation::Function(mod_id, func_id, _) => {
                            ...
                            if module.get_full_name_str().contains("vec_set") {
                                let bytecode_purity = self.recursive_bytecode_purity(env, &func);

                                if bytecode_purity {
                                    println!("Found bytecode purity in function: {}", func.get_full_name_str());
                                    continue;
                                }
                            }
    ...
    fn recursive_bytecode_purity(&self, env: &GlobalEnv, func: &FunctionEnv<'_>) -> bool {
                ...
                Bytecode::Call(_, _, operation, _, _) => {
                    match operation {
                        Operation::Function(mod_id, func_id, _) => {
                            ...
                            self.recursive_bytecode_purity(env, &new_func);
                        }
                        _ => {},
                    }
                }, 
        ...
        return self.bytecode_purity(func_bytecode);
    }
