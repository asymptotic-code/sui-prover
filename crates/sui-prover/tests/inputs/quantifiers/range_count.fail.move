...
fun test_range_count_wrong_count() {
    ...
    ensures(range_count!<u64>(0, 4, |x| is_even(x)) == 3u64.to_int());
}
...
fun test_range_count_off_by_one() {
    ...
    ensures(range_count!<u64>(0, 10, |x| is_greater_than_5(x)) == 5u64.to_int());
}
...
fun test_range_count_wrong_value_check() {
    ...
    ensures(range_count!<u64>(0, 15, |x| is_10(x)) == 2u64.to_int());
}
...
fun test_range_count_empty_not_zero() {
    ...
    ensures(range_count!<u64>(5, 5, |x| is_even(x)) == 1u64.to_int());
}
