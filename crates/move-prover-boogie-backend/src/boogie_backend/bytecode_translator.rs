    ...
    fn generate_pure_expression(
        ...
        for bytecode in code.iter() {
            match bytecode {
                    ...
                    if let [dest] = dests.as_slice() {
                        let expr = if let IfThenElse = op {
                            if let [cond, then_val, else_val] = srcs.as_slice() {
                                ...
                                panic!(
                                    "Unhandled bytecode for function translation: {:?}",
                                    bytecode
                                );
                            }
                ...
                Jump(_, _) | Branch(_, _, _, _) | Label(_, _) =>
                    // Skip control flow and other bytecodes
                    {}
                _ => {
                    // Panic on other unsupported Instructions
                    panic!(
                        "Unhandled bytecode for function translation: {:?}",
                        bytecode
                    );
            ...
            Call(_, dests, oper, srcs, aa) => {
                ...
                match oper {
                    ...
                    Function(mid, fid, inst) => {
                        ...
                        // is_pure controls whether code is emitted as a Boogie function rather than procedure.
