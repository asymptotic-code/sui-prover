...
//! Abortability analysis pass
//!
//! All functions start as potentially aborting (return type wrapped in Except).
//! This pass identifies non-aborting functions and unwraps their return types.
//!
...
use crate::{BinOp, Const, FunctionID, IRNode};
use std::collections::BTreeSet;

pub fn analyze_abortability(program: &mut Program) {
    ...
    let has_abort_bases: std::collections::HashSet<usize> =
        has_abort.iter().map(|id| id.base).collect();

    // Second pass: compute abort predicates and determine which functions can abort
    // A function can abort only if its abort predicate doesn't simplify to False
    // Process in dependency order - functions are already sorted
    let mut aborting_funcs: BTreeSet<FunctionID> = BTreeSet::new();

    for func_id in &func_ids {
        let func = program.functions.get(func_id);
        if func.is_native() {
            log::debug!("[ABORTABILITY] {} -> skipped (native)", func.name);
            continue;
        }
        if func.is_spec() {
            log::debug!("[ABORTABILITY] {} -> skipped (spec)", func.name);
            continue;
        }

        // Compute full abort predicate (handles both intrinsic aborts and calls)
        let aborting_bases: std::collections::HashSet<usize> =
            aborting_funcs.iter().map(|id| id.base).collect();
        let abort_pred =
            compute_full_abort_predicate(&func.body, &has_abort_bases, &aborting_bases, program);
        let simplified = crate::analysis::logical_simplify(abort_pred);

        let can_abort = !matches!(simplified, IRNode::Const(Const::Bool(false)));
        log::debug!(
            "[ABORTABILITY] {} -> {:?} -> can_abort={}",
            func.name,
            simplified,
            can_abort
        );
        if can_abort {
            aborting_funcs.insert(*func_id);
        }
    }

    // Collect aborting base IDs for call rewriting
    let aborting_base_ids: BTreeSet<usize> = aborting_funcs.iter().map(|id| id.base).collect();

    // Third pass: unwrap return types for non-aborting functions
    // Also rewrite calls to aborting functions to use .pure variant
    for func_id in &func_ids {
        if !aborting_funcs.contains(func_id) {
...
/// Handles both intrinsic aborts and calls to aborting functions.
fn compute_full_abort_predicate(
    body: &IRNode,
    has_abort_bases: &std::collections::HashSet<usize>,
    aborting_bases: &std::collections::HashSet<usize>,
    program: &Program,
) -> IRNode {
    let mut visited = std::collections::HashSet::new();
    compute_full_abort_predicate_impl(body, has_abort_bases, aborting_bases, program, &mut visited)
}
...
fn compute_full_abort_predicate_impl(
    ...
    // Extract abort conditions from both intrinsic aborts and aborting calls
    extract_abort_condition_full(converted, has_abort_bases, aborting_bases, program, visited)
}

/// Extract abort condition handling both intrinsic True nodes and calls to aborting functions.
fn extract_abort_condition_full(
    ...
    match node {
        ...
        // Calls to aborting functions - inline the abort predicate
        IRNode::Call {
            function,
            args,
            type_args: _,
        } if aborting_bases.contains(&function.base) => {
        ...
        IRNode::Return(values) => {
            let conditions: Vec<_> = values
                .into_iter()
                .map(|v| {
                    extract_abort_condition_full(
                        v,
                        has_abort_bases,
                        aborting_bases,
                        program,
        ...
        IRNode::Let { value, .. } => {
            extract_abort_condition_full(*value, has_abort_bases, aborting_bases, program, visited)
        }

        // While - check if condition or body can abort
        IRNode::While { cond, body, .. } => {
            let cond_abort = extract_abort_condition_full(
                *cond,
                has_abort_bases,
                aborting_bases,
                program,
            ...
            let body_abort = extract_abort_condition_full(
                *body,
                has_abort_bases,
                aborting_bases,
                program,
        ...
        IRNode::If {
            ...
            let then_abort = extract_abort_condition_full(
                *then_branch,
                has_abort_bases,
                aborting_bases,
                program,
            ...
            let else_abort = extract_abort_condition_full(
                *else_branch,
                has_abort_bases,
                aborting_bases,
                program,
