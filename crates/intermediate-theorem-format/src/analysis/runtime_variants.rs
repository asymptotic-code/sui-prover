...
/// Create a Prop constant for True (rendered as True in Lean)
fn prop_true() -> IRNode {
    // We use ToProp(true) which renders as (true = true) ≡ True
    // This ensures the constant has type Prop
    IRNode::ToProp(Box::new(IRNode::Const(Const::Bool(true))))
}
...
/// Check if a node represents Prop true constant
fn is_prop_true(node: &IRNode) -> bool {
    match node {
        IRNode::ToProp(inner) => matches!(**inner, IRNode::Const(Const::Bool(true))),
        IRNode::Const(Const::Bool(true)) => true, // legacy
        _ => false,
    }
}
    ...
    match node {
        ...
        IRNode::If {
            ...
            let (cond_bindings, pure_cond) =
                extract_aborts_with_bindings(&cond, monadic_ids, counter);
        ...
        IRNode::Call {
            ...
            args,
            type_args,
        } if monadic_ids.contains(&function.base) => IRNode::Call {
            function: FunctionID {
                base: function.base,
        ...
        match child {
            ...
            IRNode::Let { pattern, value } => {
                ...
                let (bindings, pure_value) =
                    extract_aborts_with_bindings(&value, monadic_ids, counter);
...
fn extract_aborts_with_bindings(
    ...
    match node {
        ...
        IRNode::Call {
            ...
            args,
            type_args,
        } if monadic_ids.contains(&function.base) => {
        ...
        IRNode::UnOp { op, operand } => {
            let (bindings, new_operand) =
                extract_aborts_with_bindings(operand, monadic_ids, counter);
        ...
        IRNode::Pack {
            ...
            for field in fields {
                let (bindings, new_field) =
                    extract_aborts_with_bindings(field, monadic_ids, counter);
                all_bindings.extend(bindings);
