...
pub fn render_to_directory(
    ...
    for (&module_id, module) in &program.modules {
        ...
        let (native_function_names, native_struct_names, native_namespace) =
        ...
        if needs_reexports {
            if let Some(ref ns) = native_namespace {
                for func_name in &native_function_names {
                    // Create an alias: abbrev function_name := NativeNamespace.function_name
                    module_output
                        .push_str(&format!("abbrev {} := {}.{}\n", func_name, ns, func_name));
                }
        ...
        for base_id in program.functions.iter_base_ids() {
            ...
            // Skip spec functions - they go to Specs/ folder
            if base_func.spec_target.is_some() {
                continue;
            }

            // Handle native impl functions that are spec targets
            if base_func.is_native() && is_spec_target {
                // This is a native function that is a spec target (e.g., function with while loop)
                // Render an axiom stub so the goal can reference it
                let writer = LeanWriter::new(String::new());
                let writer = super::function_renderer::render_axiom_stub(
                    base_id,
                    base_func,
                    &base_func.name,
                    program,
            ...
            if !base_func.flags.is_spec() {
                ...
                if let Some(aborts_func) = program.functions.try_get(&aborts_id) {
                    let writer = LeanWriter::new(String::new());
                    let writer =
                        render_function(aborts_id, aborts_func, program, &namespace_name, writer);
                    let rendered = writer.into_inner();
                    if !rendered.trim().is_empty() {
                        module_output.push_str(&rendered);
                        module_output.push('\n');
...
/// Render spec function bodies to Specs/ folder.
/// Spec functions are functions with `spec_target` set, meaning they provide
/// specifications for implementation functions.
fn render_spec_function_bodies(
    program: &Program,
    impls_dir: &Path,
    prelude_imports: &[String],
) -> anyhow::Result<()> {
    ...
    // Group spec functions by module for rendering
    let mut specs_by_module: std::collections::HashMap<usize, Vec<(usize, &Function)>> =
    ...
    for base_id in program.functions.iter_base_ids() {
        ...
        if base_func.spec_target.is_some() {
            specs_by_module
                .entry(base_func.module_id)
                .or_default()
                .push((base_id, base_func));
        }
    }

    // Render each module's specs to a file
    for (&module_id, specs) in &specs_by_module {
        let module = program.modules.get(module_id);
        let namespace_name = escape::module_name_to_namespace(&module.name);

        let mut output = String::new();
        output.push_str(&format!(
            "-- Spec functions for module: {}\n\n",
            module.name
        ));
        ...
        // Import the implementation module (for types)
        output.push_str(&format!(
            "import Impls.{}.{}\n",
            module.package_name, namespace_name
        ));
        ...
        // Write to Specs/<package>/<namespace>.lean
        let spec_file = specs_dir
            .join(&module.package_name)
            .join(format!("{}.lean", namespace_name));

        if let Some(parent) = spec_file.parent() {
            fs::create_dir_all(parent)?;
        }

        fs::write(spec_file, output)?;
    }
    ...
    for ((module_id, base_name), funcs) in spec_functions_by_base {
        ...
        for (func_id, _func) in &funcs {
            if let FunctionVariant::Ensures(_) = func_id.variant {
                temp_ensures_names
                    .push(escape::escape_identifier(&_func.full_name(func_id.variant)));
            }
        }
        ...
        // Import the specs module (contains spec functions like val_spec that may be called)
        spec_defs_output.push_str(&format!(
            "import Specs.{}.{}\n",
            module.package_name, namespace_name
        ));
...
fn render_spec_correct_theorem<W: Write>(
    ...
    let proof_exists =
        find_proof_in_library(proofs_dir, &func.name, package_name, namespace_name).is_some();
...
fn render_requires_proof_obligations(
    ...
    let mut spec_targets_with_requires: std::collections::HashMap<
        usize,
        (usize, &intermediate_theorem_format::Function),
    > = std::collections::HashMap::new();

    // Build target->spec mapping to find spec targets
    let target_to_spec = program.build_target_to_spec_mapping();

    for (func_id, func) in program.functions.iter() {
        if func_id.variant == FunctionVariant::Requires && func_id.is_runtime() {
            // Check if this requires is for a function that has a spec targeting it
            // The .requires variant is created for the SPEC function, so check if func has spec_target
            if let Some(target_id) = func.spec_target {
                spec_targets_with_requires.insert(target_id, (func.module_id, func));
            }
        }
    }
    ...
    for (caller_id, caller_func) in program.functions.iter() {
        ...
        if !matches!(
            caller_id.variant,
            FunctionVariant::Runtime | FunctionVariant::Pure
        ) {
            continue;
        }
        ...
        for call_id in calls {
            ...
            if let Some((callee_module_id, _requires_func)) =
                spec_targets_with_requires.get(&call_id.base)
            {
                ...
                if caller_module.package_name != callee_module.package_name
                    || caller_module.name != callee_module.name
                {
                    proof_output.push_str(&format!(
                        "import Impls.{}.{}\n",
                ...
                proof_output.push_str(&format!("-- axiom {}_obligation : True\n", proof_name));
